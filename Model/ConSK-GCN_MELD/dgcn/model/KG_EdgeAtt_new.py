...
class KG_EdgeAtt_new(nn.Module):
	...
	def __init__(self, g_dim,args,param):
		...
		self.weight_sem.data.normal_(0, var) # Gaussian distribution
	...
	def forward(self, node_features, text_len_tensor, knowledge,anew,edge_ind):
		...
		for i in range(batch_size):
			...
			for j in range(cur_len):
				...
				alphas_con[i,j, s: e + 1] = score
		
		#***************knowledge enriched attention weights***********
		knowledge_att=self.param*alphas_sem+(1-self.param)*alphas_con
